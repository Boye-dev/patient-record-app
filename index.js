...
const corsOptions = {
  origin: [
    "https://patient-record-app-production.up.railway.app",
    // "http://localhost:3000",
...
app.use("/api", require("./controllers/patient"));
app.use("/api", require("./controllers/staff"));
...
if (process.env.NODE_ENV === "production") {
  //Set static folder
  app.use(
    express.static(path.join(__dirname, "patient-record-frontend", "build"))
  );
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(__dirname, "patient-record-frontend", "build", "index.html")
    );
  });
}
